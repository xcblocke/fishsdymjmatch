import { MjGameType, EItemId } from '../../core/simulator/constant/GameTypeEnums';
import { ETileType, ETileVisible } from '../../simulator/constant/TileTypeEnum';
import DailyChallengeGameData from '../../core/simulator/data/DailyChallengeGameData';
import TravelGameModel from '../travel/model/TravelGameModel';
...
export var ECardReplaceType = {
  Wan: "wan",
  Tiao: "tiao",
  Bing: "bing"
};
export class DotUtil {
  static getInterAdPoint() {
    return cc.sys.os === cc.sys.OS_ANDROID ? "interAd" : cc.sys.os === cc.sys.OS_IOS ? "INTER" : "interAd";
  }
  static getRewardAdPoint() {
    ...
    if (e == MjGameType.Normal) {
      ...
      if (e == MjGameType.Travel) {
        ...
        if (e == MjGameType.DailyChallenge) {
          ...
          if (e == MjGameType.Classic) {
            t = 10;
          } else {
            e == MjGameType.Tile2Normal && (t = 20);
          }
        }
      }
    }
  ...
  static getGamePlayMethod() {
    return MjGameType.Tile2Normal, 0;
  }
  static isTile2Game(e) {
    return e === MjGameType.Tile2Normal;
  }
  static isClassicGame(e) {
    return e === MjGameType.Classic;
  }
  static getActivityId(e) {
    return e == MjGameType.Normal ? "-1" : e == MjGameType.Travel ? TravelGameModel.getInstance().getCurJourney() : e == MjGameType.DailyChallenge ? DailyChallengeGameData.getInstance().getDailyChallengeTimestamp().split("-").join("") : e == MjGameType.Classic ? "-2" : "unknown";
  }
  static getGameModeName(e) {
    var t = "";
    if (e == MjGameType.Normal) t = "麻将2合-关卡内-主线玩法";else if (e == MjGameType.Travel) {
      var o = TravelGameModel.getInstance().getCurJourney(),
        n = TravelGameModel.getInstance().getConfig(o);
      t = "旅行";
      n && (t = I18NStrings.getCN(n.name));
    } else if (e == MjGameType.DailyChallenge) {
      t = "每日挑战";
      t += ":" + DailyChallengeGameData.getInstance().getDailyChallengeTimestamp();
    } else e == MjGameType.Classic && (t = "无尽模式");
    return t;
  }
  static getInitialBoardDimension(e) {
    var t,
      o,
      n,
      r,
      a = 0,
      l = 0,
      s = e.getTileMapObject().mapLayers();
    try {
  ...
  static getSelfDistanceArray(e, t) {
    var o,
      n,
      a,
      l,
      ...
      u = {},
      p = e.getTileMapObject().tileObjectMap();
    try {
      for (var f = __values(p), d = f.next(); !d.done; d = f.next()) {
        var h = __read(d.value, 2),
          y = h[0];
    ...
    for (var v in u) {
      ...
      if (!(g.length < 2)) {
        ...
        for (var I in T) {
          var w = T[I];
          if (w.length >= 2) for (var B = 0; B < w.length; B++) for (var x = B + 1; x < w.length; x++) {
            var M = w[B],
              O = w[x],
              D = c(M),
              P = c(O),
              A = Math.sqrt(Math.pow(P.x - D.x, 2) + Math.pow(P.y - D.y, 2));
            _ = Math.min(_, A);
            S = true;
          }
        }
        if (!S) for (B = 0; B < g.length; B++) for (x = B + 1; x < g.length; x++) {
  ...
  static getSpecialReplaceCardIds(e) {
    var t,
      o,
      n = e.getTileMapObject().tileObjectMap(),
      a = {};
    try {
    ...
    var f = [];
    for (var u in a) f.push(u + ":" + Array.from(a[u]).join(","));
    return f;
  }
  static getFlipCardPos(e) {
    var t,
      o,
      n = [],
      a = e.getTileMapObject().tileObjectMap();
    try {
      for (var l = __values(a), c = l.next(); !c.done; c = l.next()) {
        var u = __read(c.value, 2),
          p = (u[0], u[1]);
        p.checkHasType(ETileType.RollCard) && n.push(cc.v3(p.gridPosX, p.gridPosY, p.layer));
      }
    } catch (e) {
      t = {
        error: e
  ...
  static getMahjongMatchCntArray(e) {
    var t,
      o,
      n = {},
      a = e.getTileMapObject().tileObjectMap();
  ...
  static getExposedPairInfo(e) {
    ...
    try {
      for (var A = __values(I), R = A.next(); !R.done; R = A.next()) {
        D = R.value;
        _.isHasVisible(D) !== ETileVisible.None && P.push(D);
      }
    } catch (e) {
      l = {
        error: e
  ...
  static getLeftRightCnt(e) {
    var t,
      o,
      n = 0,
      a = 0,
      l = 0,
      s = e.getTileMapObject().tileObjectMap();
    try {
      for (var c = __values(s), u = c.next(); !u.done; u = c.next()) {
  ...
  static getLevelDataAsCardId(e, t = true) {
    var o = [];
    e.tileObjectMap().forEach(function (e) {
      var n = e.pos,
        i = t ? e.isValid && !e.getIsInSlotBar() : e.isValid;
      o.push({
        id: e.cardId,
        pos: {
          x: n.x,
    ...
    return LevelGenRule.serializeTilesToInlineString(o);
  }
  static getLevelDataAsResId(e, t = true) {
    var o = [];
    e.tileObjectMap().forEach(function (e) {
      var n = e.pos,
        i = t ? e.isValid && !e.getIsInSlotBar() : e.isValid;
  ...
  static getSpecialTiles(e, t = true) {
    var o, n;
    var r = [],
      a = e.aliveTiles();
    try {
  ...
  static getTileType(e) {
    var t,
      o,
      n = Object.values(ETileType),
      r = [];
    try {
      for (var a = __values(n), l = a.next(); !l.done; l = a.next()) {
        var c = l.value;
        c !== ETileType.Normal && e.checkHasType(c) && r.push(c);
      }
    } catch (e) {
      t = {
        error: e
  ...
  static getBlockStatusPositionList(e, t = true) {
    var o, n;
    var l = e.tileObjectMap(),
      s = [];
    try {
  ...
  static getPreOpPairs(e, t) {
    var o,
      n,
      r,
      a,
      l = e.aliveTiles().filter(function (e) {
  ...
  static getOperateCardIdList(e) {
    var t,
      o,
      n = e.aliveTiles().filter(function (e) {
        return !e.getIsInSlotBar();
      }),
      r = new Set();
    try {
