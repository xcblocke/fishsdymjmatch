export class TopMaskEnterStrategy {
  ANIM_DURATION = 0.5;
  ROW_INTERVAL = 0.03;
  RANDOM_DELAY_RANGE = 0.03;
  DROP_OFFSET_Y = 200;
  totalDuration = 0;
  getName() {
    return "TopMaskEnter";
  ...
  generateAnimationConfigs(e) {
    ...
    try {
      for (var T = __values(m), C = T.next(); !C.done; C = T.next()) {
        ...
        if (cc.isValid(b.cardNode) && cc.isValid(b.shadowNode)) {
          var E = b.info.layerIndex || 0,
            S = (null === (d = b.info.tileObject) || void 0 === d ? void 0 : d.gridPosY) || 0,
            I = (null === (h = b.info.tileObject) || void 0 === h ? void 0 : h.gridPosX) || 0,
            w = b.cardNode.position.y;
          g = Math.max(g, w);
          _ = Math.min(_, w);
    ...
    if (0 === v.size) return y;
    var B = g + this.DROP_OFFSET_Y,
      x = g - _ + this.DROP_OFFSET_Y;
    try {
    ...
    var D = 0,
      P = 0,
      A = Array.from(v.keys()).sort(function (e, t) {
        return e - t;
      });
    try {
      for (var R = __values(A), N = R.next(); !N.done; N = R.next()) {
        E = N.value;
        var j = v.get(E),
          k = Array.from(j.keys()).sort(function (e, t) {
            return t - e;
          });
        try {
          for (var L = (c = void 0, __values(k)), G = L.next(); !G.done; G = L.next()) {
            ...
            try {
              for (var V = (p = void 0, __values(F)), U = V.next(); !U.done; U = V.next()) {
                var H = U.value,
                  W = (2 * Math.random() - 1) * this.RANDOM_DELAY_RANGE,
                  z = Math.max(0, P + W);
                D = Math.max(D, z);
