export class IntuitiveChainPlusCoordSel {
  calculatePriority(e, t, o) {
    var n = o.tileToCoord(e),
      ...
      c = false;
    if (r) {
      var u = this._coordManhattanDistance(n, r),
        p = this._coordManhattanDistance(i, r);
      if (u <= 3) {
        l = e;
        s = t;
        c = true;
    ...
    if (l && s) {
      var f = 1,
        d = this._tileManhattanDistance(l, s);
      d >= 6 && d <= 10 && (f *= 2);
  ...
  _coordManhattanDistance(e, t) {
    var o = e.split("_").map(Number),
      n = t.split("_").map(Number);
    return o.length < 3 || n.length < 3 ? Infinity : Math.abs(o[0] - n[0]) + Math.abs(o[1] - n[1]) + Math.abs(o[2] - n[2]);
  }
  _tileManhattanDistance(e, t) {
