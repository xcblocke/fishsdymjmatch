...
export class Tile2ClearBehavior extends GameBehaviorsBase {
  ...
  start(e) {
    var t,
      o = this,
      n = e.data.tileIds;
    if (!n || n.length < 2) this.finish(EBehaviorEnum.Success);else {
      ...
      if (r && u && r.cardNode && u.cardNode) {
        ...
        r.onClear();
        u.onClear();
        var p = this.context.gameView.nodeDragCardRoot,
          f = this.context.gameView.slotBarView;
        ...
        var h = r.tileObject.checkHasType(ETileType.RankCard),
          y = r.cardNode.convertToWorldSpaceAR(cc.v2(0, 0)),
          m = u.cardNode.convertToWorldSpaceAR(cc.v2(0, 0)),
          v = cc.v3((y.x + m.x) / 2, (y.y + m.y) / 2, 0);
        this.context.setLastCollisionWorldPos(v);
        var g = this.context.getTile2SlotType() === ETile2SlotType.Slot3,
          _ = e.data.clearType;
        if (g && _ == ETile2ClearType.OutSlotBar) {
  ...
  playKillCardAnimation(e, t, o = 216, n = 268, i?, r?) {
    var a,
      l,
      s = e.getPosition(),
      c = t.getPosition();
    ...
    var u = a.getPosition().add(l.getPosition()).mulSelf(0.5),
      p = cc.v3(u.x, u.y, 0),
      f = this.getTargetPosList(a, p, o, true),
      d = this.getTargetPosList(l, p, o, false),
      h = e.parent.convertToWorldSpaceAR(p);
    this.playKillAnimation(a, f);
    this.playKillAnimation(l, d, function () {
      return null == i ? void 0 : i(h);
    }, r);
  }
  getTargetPosList(e, t, o, n) {
    var i = n ? -1 : 1,
      r = e.getPosition().clone();
    r.x += 0.5 * o * this.halfWidthDev * i * e.scaleX;
    r.y = t.y;
    return [cc.v3(r.x, r.y), t.clone()];
  }
  playKillAnimation(e, t, o, n) {
    if (e && cc.isValid(e) && t.length) {
      var i = cc.v2(e.width, 3 * e.height).len(),
        r = e.width,
        a = cc.Vec2.distance(t[0], e.position),
        l = cc.misc.clampf((a - r) / (i - r), 0, 1),
        s = cc.misc.lerp(this.flyTimeStep1Min, this.flyTimeStep1Max, l),
        c = 3 * e.width,
        u = e.width,
        p = Math.abs(t[1].x - t[0].x),
        f = cc.misc.clampf((p - u) / (c - u), 0, 1),
        d = cc.misc.lerp(this.flyTimeStep2Min, this.flyTimeStep2Max, f);
      cc.tween(e).to(s, {
        position: t[0]
      }, {
