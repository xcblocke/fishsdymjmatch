...
export class ShuffleModifier extends BaseCoreObject {
  _shuffledCardTiles = [];
  _mapLayers = [];
  _isYoga = false;
  constructor(t) {
    ...
    for (var o = this._mapLayers.length - 1; o >= 0; o--) try {
      for (var n = (e = void 0, __values(this._mapLayers[o].allCards)), i = n.next(); !i.done; i = n.next()) {
        ...
        if (r.isValid) {
          ...
          r.resId === ResId.emYogaCardId && (this._isYoga = true);
        }
      }
    } catch (t) {
      e = {
  ...
  shuffleUnity() {
    ...
    if (c) {
      ...
      for (var d = [], h = [...this._shuffledCardTiles].sort(function (e, t) {
          return e.resId - t.resId;
        }), y = 0; y < h.length; y += 2) y + 1 < h.length && d.push([{
        resId: h[y].resId,
        type: h[y].type,
  ...
  assignResIds(e, t) {
    for (var o = 0; o < t.length && 2 * o + 1 < e.length; o++) {
      var n = __read(t[o], 2),
        ...
        a = e[2 * o],
        s = e[2 * o + 1];
      if (a.type !== ETileType.RollCard) {
        var c = i.type === ETileType.RollCard ? ETileType.Normal : i.type;
        this._context.getTileMapObject().setTileType(a.id, c);
      }
      a.changeResId(i.resId);
      a.setDuoxiaoCount(i.duoxiaoCount || 0);
      if (s.type !== ETileType.RollCard) {
        var u = r.type === ETileType.RollCard ? ETileType.Normal : r.type;
  ...
  shuffleYoga() {
    var e,
      ...
      r = [];
    try {
      for (var c = __values(this._shuffledCardTiles), f = c.next(); !f.done; f = c.next()) (b = f.value).resId === ResId.emYogaCardId && i.push(b);
    } catch (t) {
      e = {
        error: t
      };
    ...
    for (var d = this._shuffledCardTiles.filter(function (e) {
      ...
      }), h = [...d].sort(function (e, t) {
        return e.resId - t.resId;
      }), y = [], m = 0; m < h.length; m += 2) m + 1 < h.length && y.push([{
      resId: h[m].resId,
      type: h[m].type,
    ...
    for (m = 0; m < y.length && m < v.length; m++) {
      ...
      if (I && w) {
        var P = x.type === ETileType.RollCard ? ETileType.Normal : x.type,
          A = M.type === ETileType.RollCard ? ETileType.Normal : M.type;
  ...
  getCanFillSoloCollectPositions(e, t) {
    var o, n, i, r, l, s;
    try {
      if (0 === e.length) return null;
      var c = [],
        u = this.getLegalPositions(t, false);
      try {
        for (var p = __values(e), f = p.next(); !f.done; f = p.next()) {
          var d = f.value;
          if (u.includes(d)) {
            var h = this._context.getTileMapObject().getAdjacentLeftCards(d),
              y = this._context.getTileMapObject().getAdjacentRightCards(d),
              m = false,
              v = false;
            try {
  ...
  getRandomPositionPair(e, t, o = false) {
    try {
    ...
    try {
      ...
      try {
        for (var r = __values(this._shuffledCardTiles), l = r.next(); !l.done; l = r.next()) {
          var s = l.value;
          if (!e.includes(s)) {
            var c = this.checkIfSpreadingPathLegal(s, 1, false, e, t),
              u = this.checkIfSpreadingPathLegal(s, 2, false, e, t),
              p = this.isPositionDownPositionsFilled(s, e);
            if (t) {
              (c || u) && p && i.push(s);
            } else {
              c && u && p && i.push(s);
  ...
  checkIfSpreadingPathLegal(e, t, o, n, i = false) {
    var r, l;
    try {
      var s = this._context.getTileMapObject(),
        c = 1 === t ? s.getAdjacentLeftCards(e) : s.getAdjacentRightCards(e);
      if (0 === c.length) return true;
      try {
        for (var u = __values(c), p = u.next(); !p.done; p = u.next()) {
          var f = p.value;
          if (o) {
            if (n.includes(f)) return false;
            if (!this.checkIfSpreadingPathLegal(f, t, true, n, i)) return false;
          } else {
            if (i && !n.includes(f)) return false;
            var d = !n.includes(f);
            if (!this.checkIfSpreadingPathLegal(f, t, d, n, i)) return false;
          }
        }
      } catch (e) {
        r = {
  ...
  isPositionDownPositionsFilled(e, t) {
    var o, n;
    try {
  ...
  getTileAtPosition(e, t, o) {
    var n, i;
    try {
      for (var r = __values(this._shuffledCardTiles), l = r.next(); !l.done; l = r.next()) {
        var s = l.value;
        if (s.gridPosX === e && s.gridPosY === t && s.layer === o) return s;
      }
    } catch (e) {
      n = {
        error: e
