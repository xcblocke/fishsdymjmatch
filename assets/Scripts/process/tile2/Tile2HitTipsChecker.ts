...
export class Tile2HitTipsChecker extends BaseCoreObject {
  constructor(t) {
    super(t);
  }
  computeTile2Hint() {
    ...
    var i = this.getBoardTiles(e),
      r = this.getSlotCardIds(o);
    if (n > 0 && n <= 3) {
    ...
    if (n <= 1 && 0 === s.length) {
      var u;
      if (u = this.findRevealFormPair(i, e, t.slotBarCount, n)) return {
        clearId1: u.b1.id,
        clearId2: u.b2.id,
        flipId: u.flip.id
      };
  ...
  isRollLockCard(e, t) {
    return e.checkHasType(ETileType.RollCard) && 0 !== t.isCardLock(e);
  }
  getSlotCardIds(e) {
    var t,
      o,
  ...
  findSlotTileId(e, t, o) {
    var n, i;
    try {
      for (var a = __values(t), l = a.next(); !l.done; l = a.next()) {
        var s = l.value,
          c = e.getTileObjectByPosId(s);
        if ((null == c ? void 0 : c.isValid) && c.cardId === o) return c.id;
      }
    } catch (e) {
      n = {
        error: e
  ...
  getBoardPairs(e, t) {
    ...
    e.getCanMatchTiles().forEach(function (e) {
      ...
      if (t.length >= 2) for (var o = 0; o + 1 < t.length; o += 2) p(t[o], t[o + 1]);
    });
    try {
      for (var h = __values(t), y = h.next(); !y.done; y = h.next()) {
        var m = y.value;
        if (m.isValid && f(m) && d(m)) {
          var v = this.getAllCoverCards(m, e),
            g = this.dedupTiles(e.getAdjacentLeftCards(m)),
            _ = this.dedupTiles(e.getAdjacentRightCards(m));
          if (!(g.length > 0 && _.length > 0 && v.length > 0)) if (g.length > 0 && _.length > 0) try {
            for (var T = (i = void 0, __values([...g, ..._])), C = T.next(); !C.done; C = T.next()) {
              var b = C.value;
              if (f(b) && b.cardId === m.cardId && !d(b)) {
                var E = 1 === g.length && g[0].id === b.id,
                  S = 1 === _.length && _[0].id === b.id;
                (E || S) && p(b, m);
              }
            }
          } catch (e) {
  ...
  findRevealMatchHolder(e, t, o, n, i) {
    ...
    var C = Math.min.apply(Math, [...m.map(function (e) {
  ...
  calcMinRevealCount(e, t, o) {
    ...
    var v = 0,
      g = this.getAllCoverCards(e, t);
    try {
      for (var _ = __values(g), T = _.next(); !T.done; T = _.next()) {
        var C = T.value;
        if ((v += this.calcMinRevealCount(C, t, o) + 1) >= 999) break;
      }
    } catch (e) {
      n = {
        error: e
    ...
    if (v < 999) {
      var b = this.dedupTiles(t.getAdjacentLeftCards(e)),
        E = this.dedupTiles(t.getAdjacentRightCards(e));
      if (b.length > 0 && E.length > 0) {
        var S = 0;
        try {
          for (var I = __values(b), w = I.next(); !w.done; w = I.next()) {
            var B = w.value;
            if ((S += this.calcMinRevealCount(B, t, o) + 1) >= 999) break;
          }
        } catch (e) {
          a = {
            error: e
        ...
        try {
          for (var M = __values(E), O = M.next(); !O.done; O = M.next()) {
            var D = O.value;
            if ((x += this.calcMinRevealCount(D, t, o) + 1) >= 999) break;
          }
        } catch (e) {
          s = {
            error: e
            ...
            for (var R = __values([b, E]), N = R.next(); !N.done; N = R.next()) {
              ...
              try {
                for (var L = (f = void 0, __values(j)), G = L.next(); !G.done; G = L.next()) {
                  ...
                  if ((k += Math.max(0, this.calcMinRevealCount(F, t, o) - U) + 1) >= 999) break;
                }
              } catch (e) {
                f = {
                  error: e
  ...
  findClickableBlockers(e, t, o, n) {
    var i, a, s, c, u, p, f, d;
    n || (n = new Set());
    if (n.has(e.id)) return [];
    n.add(e.id);
    if (0 === t.isCardLock(e)) return [e];
    var h = [];
    try {
      for (var y = __values(this.getAllCoverCards(e, t)), m = y.next(); !m.done; m = y.next()) {
        var v = m.value;
        h.push.apply(h, [...this.findClickableBlockers(v, t, o, n)]);
      }
    } catch (e) {
      i = {
        error: e
    ...
    var g = this.dedupTiles(t.getAdjacentLeftCards(e)),
      _ = this.dedupTiles(t.getAdjacentRightCards(e));
    if (g.length > 0 && _.length > 0) {
      var T = g.some(function (e) {
        ...
        b = void 0;
      if (T && C) b = null;else if (T) b = _;else if (C) b = g;else {
        var E = 0;
        try {
      ...
      if (b) try {
        for (var D = __values(b), P = D.next(); !P.done; P = D.next()) {
          var A = P.value;
          h.push.apply(h, [...this.findClickableBlockers(A, t, o, n)]);
        }
      } catch (e) {
        f = {
          error: e
  ...
  getAllCoverCards(e, t) {
    var o,
      n,
      i = [],
      a = new Set(),
      l = t.mapLayers().length;
    try {
      for (var s = __values(e.ownerGridIds), c = s.next(); !c.done; c = s.next()) for (var u = c.value, p = e.layer + 1; p < l; p++) if (t.isHasCard(p, u)) {
  ...
  dedupTiles(e) {
    ...
    return e.filter(function (e) {
      if (t.has(e.id)) return false;
      t.add(e.id);
      return true;
    });
  }
  scoreRevealMatchHolder(e) {
    var t = 0;
    this.isRollCard(e) || (t += 100);
    return t + this.countNewPairsAfterHint([e.id]);
  }
  findRevealFormPair(e, t, o, n) {
    var i,
      s,
      c,
      u,
    ...
    try {
      for (var w = __values(_), B = w.next(); !B.done; B = w.next()) {
        ...
        if (!(x.length < 2)) for (var M = 0; M < x.length; M++) for (var O = M + 1; O < x.length; O++) {
          var D = x[M],
            P = x[O],
            A = this.calcMinRevealCount(D, t, S),
            R = this.calcMinRevealCount(P, t, S);
          if (!(A >= 999 && R >= 999)) {
    ...
    var j = Math.min.apply(Math, [...I.map(function (e) {
        return e.revealCount;
      })]),
  ...
  calcPairRevealCount(e, t, o, n, i, a) {
    var l,
      s,
      c,
      u,
      ...
      f,
      d = o + n,
      h = this.collectBlockerSets(e, i, a),
      y = this.collectBlockerSets(t, i, a);
    try {
  ...
  collectBlockerSets(e, t, o) {
    var n, i, a, l, s, c;
    if (0 === t.isCardLock(e)) return [new Set()];
    var u = this.getAllCoverCards(e, t),
      p = this.dedupTiles(t.getAdjacentLeftCards(e)),
      f = this.dedupTiles(t.getAdjacentRightCards(e)),
      d = new Set(),
      h = new Set([e.id]);
    try {
    ...
    if (0 === p.length || 0 === f.length) return [d];
    var g = [];
    try {
  ...
  isRollCard(e) {
    return e.type === ETileType.RollCard;
  }
  countNewPairsAfterHint(e) {
    var t, o;
    try {
      var n = TileMapSimulator.createSim(this._context);
      try {
        for (var i = __values(e), a = i.next(); !a.done; a = i.next()) {
          var l = a.value,
            s = n.tileObjectMap().get(l);
