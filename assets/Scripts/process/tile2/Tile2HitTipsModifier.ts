...
export class Tile2HitTipsModifier extends BaseCoreObject {
  clearAllHitTips() {
    var e,
      t,
      o = this.context.getCanHitTips() || [];
    try {
  ...
  tryClearHitTips(e) {
    ...
    var a = this.context.getTileMapObject().getTileObject(e[0]),
      l = this.context.getTileMapObject().getTileObject(e[1]);
    if (!a || !l) return false;
    var s = this.context.getCanHitTips() || [];
    try {
      for (var c = __values(s), u = c.next(); !u.done; u = c.next()) {
        var p = u.value,
          f = this.context.getTileMapObject().getTileObject(p);
        if (f) {
          var d = f.cardId;
  ...
  modifyClearHitTips(e) {
    ...
    try {
      for (var i = __values(e), a = i.next(); !a.done; a = i.next()) {
        var l = a.value;
        if (this.tryClearHitTips(l)) return {
          clearHit: true,
          hideHitIds: n
        };
      }
