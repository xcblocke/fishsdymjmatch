...
export class AcAnimPlayerBase {
  ...
  arcTo(e, t, o, n) {
    return new Promise(function (i) {
      if (cc.isValid(e)) {
        var r = e.position.clone(),
          a = t,
          l = a.x - r.x,
          s = a.y - r.y,
          c = Math.sqrt(l * l + s * s),
          u = c * (0.6 + 0.2 * Math.random()),
          p = Math.random() > 0.5,
          f = (r.x + a.x) / 2,
          d = (r.y + a.y) / 2,
          h = f + -s / c * u * (p ? 1 : -1),
        ...
        cc.tween(e).to(o, {
          ...
          progress: function (t, o, i, l) {
            var s = l < 0.5 ? 2 * l * l : 1 - Math.pow(-2 * l + 2, 2) / 2,
              c = (1 - s) * (1 - s) * r.x + 2 * (1 - s) * s * h + s * s * a.x,
              u = (1 - s) * (1 - s) * r.y + 2 * (1 - s) * s * y + s * s * a.y;
            cc.isValid(e) && (e.position = cc.v3(c, u, 0));
            if (n && cc.isValid(n)) {
              var p = 2 * (1 - s) * (h - r.x) + 2 * s * (a.x - h),
                f = 2 * (1 - s) * (y - r.y) + 2 * s * (a.y - y);
  ...
  createGraphicsMask(e = "AcMaskLayer") {
  ...
  showFlowLightNode(e, t, o, i = -1) {
    ...
    if (!e.getChildByName("DaxiaoFlowLightNode")) {
      ...
      BaseSpine.refreshWithNode(a, t, o).setAnimation("init", i, null, false);
    }
  }
  cleanupAndComplete() {
