export class ChainAvoidCharSel {
  selectCharacterPair(e, t, o, a) {
    var l,
      s,
      c,
      u,
      ...
      _ = a.tileToCoord(e),
      T = a.tileToCoord(t),
      C = a.getExpanded("pred", _),
      b = a.getExpanded("pred", T),
      E = new Set(),
      S = new Set();
    try {
    ...
    if (te.length > 0) {
      if (0 === J.size) {
        ...
        var oe = re[0].count,
          ne = re.filter(function (e) {
            return e.count === oe;
          }).map(function (e) {
            return e.index;
          });
        ee = ne[Math.floor(Math.random() * ne.length)];
      } else ee = te[Math.floor(Math.random() * te.length)];
    } else if (J.size > 0) {
      ...
      if (ie.length > 0) {
        ...
        var ae = re[0].count,
          le = re.filter(function (e) {
            return e.count === ae;
          }).map(function (e) {
            return e.index;
          });
        ee = le[Math.floor(Math.random() * le.length)];
      } else ee = j[Math.floor(Math.random() * j.length)];
    } else ee = j[Math.floor(Math.random() * j.length)];
    return ee;
  }
}