...
export default class Outlight extends cc.Component {
  ...
  _lightOpaque = 2;
  @property
  _lightColor = cc.Color.BLACK;
  @property
  _offset = cc.Vec2.ZERO;
  @property
  _lightAlphaThreshold = 1;
  isBaked = false;
  _isLowEndLimited = false;
  _bakedRT = null;
  _bakeCamera = null;
  _rebaking = false;
  _bakeNode = null;
  _originalParent = null;
  static isBakedDefault = false;
  static isLimitLowEndDevice = true;
  @property({
    type: cc.Integer,
    range: [0, 20],
    slide: true,
  ...
  onLoad() {
    var e = this;
    this.isBaked = Outlight.isBakedDefault;
  ...
  LowEndDeviceLimit() {
    if (!this.isBaked && Outlight.isLimitLowEndDevice && 1 / cc.director.getDeltaTime() < 40) {
  ...
  rebake() {
    var e = this;
    if (this.isBaked && this.enabledInHierarchy && !this._isLowEndLimited) if (this._rebaking) this.scheduleOnce(function () {
      e.rebake();
    }, 0);else {
      this._rebaking = true;
      this.scheduleOnce(function () {
  ...
  _doRebake() {
    if (this.isBaked && this.enabledInHierarchy && this.node.isValid) {
      ...
      var r = this.node.getPosition(),
        a = this.node.getAnchorPoint(),
        l = this.node.getScale(cc.v3()),
        s = this.node.opacity,
        c = this.node.angle,
        u = this._originalParent.getChildByName("bakedNode");
      if (!u) {
      ...
      u.setScale(l);
      u.angle = c;
      var p = u.getComponent(cc.Sprite),
        f = null;
      if (p) f = p.spriteFrame;else {
    ...
    } else this._rebaking = false;
  }
  onEnable() {
    var e = this;
    if (!this._isLowEndLimited) if (this._renderComponent) this.updateProperties();else {
      ...
      resManager.loadRes("materials/Outlight", cc.Material, "resources").then(function (e) {
        t(e);
      });
    }
  }
