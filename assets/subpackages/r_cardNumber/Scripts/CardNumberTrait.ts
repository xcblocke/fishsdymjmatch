...
export default class CardNumberTrait extends Trait {
  _currentMapping = {};
  static traitKey = "CardNumber";
  ...
  _generateRandomMapping() {
    var t,
      r = {},
      a = CardNumberTrait.ALL_CARDS.slice(0, 18).filter(function (t) {
        return t && t.length > 0;
      });
    if (18 !== a.length) return r;
    var n = this._getTaskCardResName(),
      o = CardNumberTrait.ALL_CARDS.filter(function (t) {
        return t && t.length > 0;
      });
  ...
  onGsListener_onNewGame(t, r) {
    try {
      ...
      console.error("[" + CardNumberTrait.traitKey + "] 关卡切换处理失败: " + (null == t ? void 0 : t.message));
    }
    r();
  }
    ...
    try {
      ...
      if (i) {
        ...
        r({
          ...
          returnVal: {
            path: f,
            bundleName: CardNumberTrait.BUNDLE_NAME,
            fromAtlas: true
          }
        });
        return;
      ...
      console.error("[" + CardNumberTrait.traitKey + "] 劫持图片失败: " + (null == t ? void 0 : t.message));
      r();
    }
  }
}