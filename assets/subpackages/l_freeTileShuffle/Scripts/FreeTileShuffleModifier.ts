import { BaseCoreObject } from '../../../Scripts/BaseCoreObject';
import { LayoutSelector } from '../../../Scripts/process/layout/LayoutSelector';
import { ResId, MjGameType } from '../../../Scripts/core/simulator/constant/GameTypeEnums';
export class FreeTileShuffleModifier extends BaseCoreObject {
  _mapLayers = [];
  _shuffleOriginalPositions = new Map();
  _tileMapObject = null;
  constructor(t) {
    super(t);
    this._tileMapObject = t.getTileMapObject();
    this._mapLayers = this._tileMapObject.mapLayers();
  }
  getShuffleOriginalPositions() {
    return this._shuffleOriginalPositions;
  }
  executeFreeTileShuffle() {
    Date.now();
    var e,
      t,
      r,
      i,
      o = this._tileMapObject.aliveTiles();
    if (0 === o.length) return {
      sequence: [],
      freeTiles: []
    };
    var n = this._context.getTileMapObject(),
      s = n.mapLayers(),
      c = new Set(o);
    try {
  ...
  executeYogaTileShuffle() {
    ...
    o.filter(function (e) {
      return e.resId === ResId.emYogaCardId;
    }).length;
    this.saveOriginalPositions(o);
  ...
  generatePairingWithFreeTiles(e) {
  ...
  extractFreeTiles(e, t, r) {
    for (;;) {
      var i = this.getSelectableTiles(t);
      if (0 === i.length) break;
      if (i.length > 1 && this.findSafePair(i, t)) break;
      var o = this.selectTileToExtract(i, t);
      r.push(o);
      t.delete(o);
      var a = this.getSelectableTiles(t);
      if (a.length > 1 && this.findSafePair(a, t)) break;
      if (t.size < 2) break;
    }
  }
  selectTileToExtract(e, t) {
    var r, i;
  ...
  findSafePair(e, t) {
    var r = [...e];
    this.shuffleArray(r);
    for (var i = 0; i < r.length; i++) for (var o = i + 1; o < r.length; o++) {
      var a = r[i],
        n = r[o];
      if (this.isSafePair(a, n, t)) return [a, n];
    }
    return null;
  }
  isSafePair(e, t, r) {
    if (r.size - 2 <= 1) return true;
    var i = this.getSelectableTiles(r),
      o = i.length,
      a = 0;
    i.includes(e) && a++;
    i.includes(t) && a++;
  ...
  countFreedTilesForPair(e, t, r) {
    var i,
      o,
      n = this.tileToCoord(e),
      l = this.tileToCoord(t),
      s = 0;
    try {
  ...
  isTileSelectable(e, t) {
    if (this.hasCover(e, t)) return false;
    var r = this.hasLeftNeighbor(e, t),
      i = this.hasRightNeighbor(e, t);
    return !(r && i);
  }
  wouldBecomeSelectable(e, t, r, i) {
    if (this.hasCoverExcluding(e, i, t, r)) return false;
    var o = this.hasLeftNeighborExcluding(e, i, t, r),
      a = this.hasRightNeighborExcluding(e, i, t, r);
    return !(o && a);
  }
  hasCover(e, t) {
    var r,
      i,
      o = this.getAdjacentUpperCards(e);
    try {
  ...
  hasCoverExcluding(e, t, r, i) {
    var o,
      n,
      l = this.getAdjacentUpperCards(e);
    try {
      for (var s = __values(l), c = s.next(); !c.done; c = s.next()) {
        var f = c.value,
          u = this.tileToCoord(f);
        if (u !== r && u !== i && t.has(f)) return true;
      }
    } catch (e) {
      o = {
        error: e
  ...
  hasLeftNeighbor(e, t) {
    var r,
      i,
      o = this._tileMapObject.getAdjacentLeftCards(e);
    try {
      for (var n = __values(o), l = n.next(); !l.done; l = n.next()) {
        var s = l.value;
        if (t.has(s)) return true;
  ...
  hasRightNeighbor(e, t) {
    var r,
      i,
      o = this._tileMapObject.getAdjacentRightCards(e);
    try {
      for (var n = __values(o), l = n.next(); !l.done; l = n.next()) {
        var s = l.value;
        if (t.has(s)) return true;
  ...
  hasLeftNeighborExcluding(e, t, r, i) {
    var o,
      n,
      l = this._tileMapObject.getAdjacentLeftCards(e);
    try {
      for (var s = __values(l), c = s.next(); !c.done; c = s.next()) {
        var f = c.value,
          u = this.tileToCoord(f);
  ...
  hasRightNeighborExcluding(e, t, r, i) {
    var o,
      n,
      l = this._tileMapObject.getAdjacentRightCards(e);
    try {
      for (var s = __values(l), c = s.next(); !c.done; c = s.next()) {
        var f = c.value,
          u = this.tileToCoord(f);
  ...
  getAdjacentUpperCards(e) {
    var t,
      r,
      i = [],
      o = e.layer + 1;
    if (o >= this._mapLayers.length) return i;
    try {
    ...
    if (0 !== e.length) {
      ...
      try {
        for (var c = __values(this._mapLayers), u = c.next(); !u.done; u = c.next()) {
          var h = u.value;
          try {
            for (var p = (i = void 0, __values(h.allCards)), d = p.next(); !d.done; d = p.next()) if ((T = d.value).isValid && !s.has(T)) {
      ...
      for (var y = Math.max(0, l), v = n + 2, g = 0; g < e.length; g++) {
        var T = e[g],
          m = y + g % 7 * 2,
          _ = v + 2 * Math.floor(g / 7);
        this._tileMapObject.moveTileToPosition(T, m, _, 0);
      }
      this._context.gameType == MjGameType.Classic && this._context.getTileMapObject().applyCenterOffsetToAllTiles();
    }
  }
  handleFloatingTiles() {
    for (var e, t, r = this._mapLayers.length - 1; r > 0; r--) {
      var i = this._mapLayers[r],
        o = [...i.allCards];
      try {
  ...
  isCompletelyUnsupported(e) {
    ...
    try {
      for (var o = __values(e.ownerGridIds), n = o.next(); !n.done; n = o.next()) {
        var l = n.value;
        if (this._tileMapObject.isHasCard(i, l)) return false;
      }
    } catch (e) {
      t = {
        error: e
  ...
  recalculateLayoutScale() {
    var e = this._context.getContentSize(),
      t = new LayoutSelector(this._context).chooseLayout({
        contentSize: new cc.Size(e.width, e.height)
      });
    this._context.getLayoutScale();
    this._context.gameType != MjGameType.Classic && this._context.setLayoutScale(t.scale);
