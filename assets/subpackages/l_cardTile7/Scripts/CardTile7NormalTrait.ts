...
export default class CardTile7NormalTrait extends Trait {
  _currentMapping = {};
  static traitKey = "CardTile7Normal";
  ...
  _generateRandomMapping() {
    var t,
      r = {},
      a = CardTile7NormalTrait.ALL_CARDS.slice(0, 18).filter(function (t) {
        return t && t.length > 0;
      });
    if (18 !== a.length) return r;
    var n = this._getTaskCardResName(),
      i = CardTile7NormalTrait.ALL_CARDS.filter(function (t) {
        return t && t.length > 0;
      });
  ...
  onAdMgr_interAdClose(t, r) {
    try {
      ...
      console.error("[" + CardTile7NormalTrait.traitKey + "] 插屏广告关闭处理失败: " + (null == t ? void 0 : t.message));
    }
    r();
  }
  onGsListener_onNewGame(t, r) {
    try {
      ...
      console.error("[" + CardTile7NormalTrait.traitKey + "] 关卡切换处理失败: " + (null == t ? void 0 : t.message));
    }
    r();
  }
  onCardUtil_atlasPathBundle(t, r) {
    ...
    try {
      ...
      console.error("[" + CardTile7NormalTrait.traitKey + "] 劫持图片失败: " + (null == t ? void 0 : t.message));
      r();
    }
  }
}