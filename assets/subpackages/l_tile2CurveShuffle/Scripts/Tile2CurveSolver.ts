import { DependencyGraph } from '../../../Scripts/DependencyGraph';
import { SafetyChecker } from '../../../Scripts/SafetyChecker';
import { DEFAULT_CURVE_PARAMS } from './ICurveSolverTypes';
this && this.__spread;
@mj.class("Tile2CurveSolver")
export default class Tile2CurveSolver {
  graph = new DependencyGraph();
  safety = new SafetyChecker(this.graph);
  board = new Map();
  remainingTiles = new Set();
  tileMap = new Map();
  holder = [];
  upperIndex = 0;
  lowerIndex = 0;
  ...
  occupancyCurve = [];
  updateCurve = [];
  params = Object.assign({}, DEFAULT_CURVE_PARAMS);
  globalStep = 0;
  holderCardIds = [];
  patternCounts = {
    "++": 0,
  ...
  fastPairCount = 0;
  shuffleWithContext(e, r) {
    var t,
      i,
      n,
      l,
      s = e.getTileMapObject(),
      h = e.getGameData(),
      c = [],
      u = new Set(h.slotBarIds || []);
    ...
    var M = h.slotBarCount || 4,
      x = this.solve(c, p, S, Object.assign({
        holderCapacity: M
      }, r));
    if (!x) return false;
  ...
  solve(e, r, t, o) {
    ...
    var i = r.length;
    if (!this.stepMatchHolder(r)) return null;
    if (0 === this.remainingTiles.size) return this.buildResult(t);
  ...
  reset(e, r) {
    var t, i, n, l;
    this.params = Object.assign(Object.assign({}, DEFAULT_CURVE_PARAMS), r);
    this.board.clear();
  ...
  stepGenerateCurves(e, r) {
    this.occupancyCurve = this.generateOccupancyCurve(r, e);
    this.updateCurve = this.generateUpdateCurve(this.occupancyCurve);
  }
  generateOccupancyCurve(e, r) {
    var t = this.params.holderCapacity - 1,
      o = e >= this.params.minStepsForPeak,
      i = [],
      a = 0,
      n = Math.max(0, 2 * (this.params.holderCapacity - r));
    if (n > 0) for (var l = Math.min(n, e - 2), s = 0; s < l; s++) {
      a = s % 2 == 0 ? 1 : 0;
      i.push(a);
    }
    ...
    for (; i.length < e;) if (i.length === e - 1) {
      i.push(0);
      a = 0;
    } else {
      a = this.pickRandomStep(a, t, e - i.length - 1);
  ...
  pickRandomStep(e, r, t) {
    var o = e + 1 <= r && this.canReachZero(e + 1, t),
      i = e - 1 >= 0 && this.canReachZero(e - 1, t);
    if (o && i) {
      var a = this.params.holderCapacity * this.params.avgOccupancyRate,
        n = Math.max(0.15, Math.min(0.85, 0.5 + 0.2 * (a - e)));
  ...
  stepComputeSolver() {
    for (var e = this.occupancyCurve, r = this.updateCurve, t = 0; t < e.length; t += 2) {
      if (this.allTilesColored()) return this.fastPairRemaining();
      if (t + 1 >= e.length) return this.processSingleStep(t);
      var o = t > 0 ? e[t - 1] : 0,
        i = (e[t] - o > 0 ? "+" : "-") + (e[t + 1] - e[t] > 0 ? "+" : "-"),
        a = r[t + 1] || 0;
      this.patternCounts[i]++;
      if (!this.processTwoSteps(i, a, t)) return false;
    }
    return true;
  }
  processTwoSteps(e, r, t) {
    switch (e) {
      case "++":
  ...
  doMinusMinus(e) {
    if (this.holder.length < 2) return false;
    var r = this.getSelectableCoords();
    if (r.length < 2) return false;
    var t = this.holder[this.holder.length - 1].colorTag,
      o = this.holder.findIndex(function (e) {
        return e.colorTag !== t;
      }),
      i = -1 !== o ? this.holder[o].colorTag : t,
      a = this.pickForPop(r);
  ...
  doPlusMinus(e, r) {
    ...
    if (0 === r) {
      ...
      this.advanceStep();
      if (0 === (a = this.getSelectableCoords()).length) return false;
      this.popWithTag(this.pickMinAge(a), o, e + 1);
    } else {
      var a,
        n = this.pickHolderColor(),
        l = (o = this.nextUpper(), this.pickMaxFreed(t));
      ...
      this.advanceStep();
      if (0 === this.holder.length) return false;
      if (0 === (a = this.getSelectableCoords()).length) return false;
  ...
  doMinusPlus(e, r) {
    ...
    var o = this.pickHolderColor(),
      i = this.nextUpper(),
      a = 0 === r ? this.pickMaxAge(t) : this.pickMinAge(t);
  ...
  fastPairRemaining() {
    for (var e = this;;) {
      ...
      var t = r.filter(function (r) {
          var t,
            o = null === (t = e.board.get(r)) || void 0 === t ? void 0 : t.colorTag;
  ...
  buildResult(e) {
    var r,
      ...
      l = new Map();
    try {
      for (var s = __values(this.board), h = s.next(); !h.done; h = s.next()) (f = __read(h.value, 2)[1]).colorTag && l.set(f.colorTag, (l.get(f.colorTag) || 0) + 1);
    } catch (e) {
      r = {
        error: e
      };
    ...
    var c = this.mapVirtualToActual(e, l),
      u = new Map();
    try {
  ...
  mapVirtualToActual(e, r) {
    ...
    var k = this.params.preferRemaining01,
      F = this.params.preferRemainingMin23,
      O = Math.floor(this.params.holderCapacity / 2),
    ...
    try {
      for (var E = __values(A.concat(j)), D = E.next(); !D.done; D = E.next()) U(b = D.value);
    } catch (e) {
      h = {
        error: e
      };
  ...
  applyResult(e, r) {
    ...
    try {
      for (var h = __values(e.coordToCardId), c = h.next(); !c.done; c = h.next()) {
        var u = __read(c.value, 1)[0],
          p = this.tileMap.get(u);
        if (p) {
          var d = p.cardId;
          s.has(d) || s.set(d, []);
          s.get(d).push(p);
    ...
    try {
      for (var v = __values(e.coordToCardId), g = v.next(); !g.done; g = v.next()) {
        var y = __read(g.value, 2),
          m = (u = y[0], y[1]),
          C = this.tileMap.get(u);
        if (C) {
          var T = s.get(m);
          T && 0 !== T.length && f.push({
            tileId: T.pop().id,
  ...
  pickMaxFreedThenMedianAge(e) {
    ...
    var t = e.map(function (e) {
      ...
      i = Math.floor(o.length / 2),
      a = o.length % 2 == 0 ? (o[i - 1] + o[i]) / 2 : o[i];
    t.sort(function (e, r) {
  ...
  pickForPop(e) {
    var r = this,
      ...
      o = e.filter(function (e) {
        var t,
          o = null === (t = r.board.get(e)) || void 0 === t ? void 0 : t.colorTag;
        return o && o === o.toLowerCase();
      }),
      i = t.length > 0 ? t : o.length > 0 ? o : e,
      a = i.filter(function (e) {
        var t,
          o = (null === (t = r.board.get(e)) || void 0 === t ? void 0 : t.age) || 0;
        return 1 === o || 2 === o;
      }),
      n = a.length > 0 ? a : i;
    return n[Math.floor(Math.random() * n.length)];
  }
  popWithTag(e, r, t, o = false) {
  ...
  allTilesColored() {
    ...
    try {
      for (var t = __values(this.board), o = t.next(); !o.done; o = t.next()) {
        ...
        if (!i.removed && !i.colorTag) return false;
      }
    } catch (r) {
      e = {
        error: r
