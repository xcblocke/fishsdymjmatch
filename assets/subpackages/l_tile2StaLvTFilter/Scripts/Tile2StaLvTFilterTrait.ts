...
function c(r) {
  var t,
    e,
    n = {};
  if (!r || "object" != typeof r || Array.isArray(r)) return n;
  var i = r;
  try {
...
function s(r, t) {
  ...
  try {
    for (var i = __values(Object.keys(t)), a = i.next(); !a.done; a = i.next()) {
      ...
      if (null != y) {
        var v = Number(y);
        if (Number.isNaN(v) || v < c || v > s) return false;
      }
    }
  } catch (r) {
    e = {
...
export default class Tile2StaLvTFilterTrait extends Trait {
  _filterMap = new Map();
  _cache = new Map();
  static traitKey = "Tile2StaLvTFilter";
  onLoad() {
    var t, e;
    super.onLoad.call(this);
    var n,
      i = this._traitData.conflictParams;
    if (i && i.levelTypes && i.filter) this._filterMap = this.mergeConflictFilters(i.levelTypes, i.filter);else {
  ...
  mergeConflictFilters(r, t) {
    for (var e, n, i, a, o, f, u, s, y = new Map(), v = Math.min(r.length, t.length), h = new Set(), p = 0; p < v; p++) if (Array.isArray(r[p])) try {
    ...
    try {
      for (var g = __values(h), m = g.next(); !m.done; m = g.next()) {
        b = m.value;
        var w = null;
        for (p = 0; p < v; p++) if (Array.isArray(r[p]) && !(r[p].indexOf(b) < 0)) {
          var x = c(t[p]);
          if (w) try {
            for (var T = (u = void 0, __values(Object.keys(x))), O = T.next(); !O.done; O = T.next()) if (w[S = O.value]) {
  ...
  onT2StaLvT_filterPool(r, t) {
    var e = r.beforReturnVal,
      n = r.args && r.args.length > 1 ? Number(r.args[1]) : 0,
      i = r.args && r.args.length > 2 ? String(r.args[2]) : "";
    if (e && 0 !== e.length && n) {
