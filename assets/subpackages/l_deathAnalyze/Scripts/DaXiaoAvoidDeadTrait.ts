...
export default class DaXiaoAvoidDeadTrait extends Trait {
  static traitKey = "DaXiaoAvoidDead";
  onDaxiaoCkr_fixFinal(t, e) {
    var r,
      n,
      i = null === (r = t.ins) || void 0 === r ? void 0 : r.context;
    if (i) {
      var a = mj.getClassByName("DeathAnalyserTrait");
      if (a && a.getInstance() && a.getInstance().eventEnabled) {
        var l = DeathAnalyserMgr.instance.analyzeCardPair(null === (n = t.ins) || void 0 === n ? void 0 : n.gameContext);
        if (l.levelAllDeathPairs.length <= 0) e();else {
          var u = __read(t.args, 3),
            c = u[0],
            s = u[1],
            d = u[2];
  ...
  fixFinalMatchInfos(t, e, r, n) {
    for (var i, a, u, c, f = t.getTileMapObject(), s = [], d = new Set(), y = 0; y < e.length; y++) {
      var h = e[y],
        p = f.getTileObject(h.tileId1),
        v = f.getTileObject(h.tileId2);
      if (p && v) if (this.isDeathGroup([p, v], n)) s.push(y);else {
        d.add(h.tileId1);
        d.add(h.tileId2);
      }
    }
    if (!(s.length <= 0)) {
      ...
      var j = f.aliveTiles(),
        w = this.findMatchTile(f, s, j, d, D, n);
      if (w.length == s.length) for (y = 0; y < w.length; y++) {
        var O = w[y];
        h = e[s[y]];
        e[s[y]] = {
          tileId1: O[0].id,
  ...
  findMatchTile(t, e, r, n, i, a) {
    for (var o = this, c = r.filter(function (t) {
        return !n.has(t.id) && t.type == ETileType.Normal;
